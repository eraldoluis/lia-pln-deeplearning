...
"""
The architecture of this neural network was based on paper Tagging The Web: Building A Robust Web Tagger with Neural Network.
This NN has as base the normal architecture of WNN.
It receives as input the word embedding of the words, features learned by the convolution of the characters or/and
sparse features of suffix and capitalization (these two last features are normally used separately).
These features are concatenated in a only vector and they are the input of a hidden layer or softmax layer.
If the hidden layer exists, so its output is the input of the softmax layer.
This softmax layer also receives as input the output from target feature module, which learns to extract features of the target
domain using unlabeled data from target.
"""
...
WNN_PARAMETERS = {
    ...
    "word_embedding_source": {"required": True,
                              "desc": "a list which contains the filters. Each filter is describe by your module name + . + class name"},

    "word_embedding_intermediary": {"required": False,
                                    "desc": "a list which contains the filters. Each filter is describe by your module name + . + class name"},
...
def createAverageIntermediary(wordEmbeddingSource, wordEmbeddingTarget):
    lexiconSource = wordEmbeddingSource.getLexicon()
    lexiconTarget = wordEmbeddingTarget.getLexicon()

    wordSet = set(lexiconSource.getLexiconList())
    wordSet.union(lexiconTarget.getLexiconList())

    wordSet.remove("UUUNKKK")

    lexiconIntermediary = Lexicon(name="word_embedding_intermediary")
    vectors = []

    vectors.append((wordEmbeddingSource.getEmbedding("UUUNKKK") + wordEmbeddingTarget.getEmbedding("UUUNKKK")) / 2)

    for word in wordSet:
        lexiconIntermediary.put(word)
        vectors.append((wordEmbeddingSource.getEmbedding(word) + wordEmbeddingTarget.getEmbedding(word)) / 2)

    lexiconIntermediary.stopAdd()

    return lexiconIntermediary, Embedding(lexiconIntermediary, vectors)
...
def mainWnn(args):
    ...
    parametersToSaveOrLoad = {"word_filters", "suffix_filters", "char_filters", "cap_filters",
                              ...
                              "use_capitalization", "start_symbol", "end_symbol", "with_hidden", "target_filters"}
    ...
    if args.load_model:
        wordLexiconSource = Lexicon.fromPersistentManager(persistentManager, "word_lexicon_source")
        vectorsSource = EmbeddingLayer.getEmbeddingFromPersistenceManager(persistentManager,
                                                                          "word_embedding_layer_source")
        wordEmbeddingSource = Embedding(wordLexiconSource, vectorsSource)

        wordLexiconTarget = Lexicon.fromPersistentManager(persistentManager, "word_lexicon_target")
        vectorsTarget = EmbeddingLayer.getEmbeddingFromPersistenceManager(persistentManager,
                                                                          "word_embedding_layer_target")
        wordEmbeddingTarget = Embedding(wordLexiconTarget, vectorsTarget)

        wordLexiconIntermediary = Lexicon.fromPersistentManager(persistentManager, "word_lexicon_intermediary")
        vectorsIntermediary = EmbeddingLayer.getEmbeddingFromPersistenceManager(persistentManager,
                                                                                "word_embedding_layer_intermediary")
        ...
        wordLexiconSource, wordEmbeddingSource = Embedding.fromWord2Vec(args.word_embedding_source, "UUUNKKK",
                                                                        "word_lexicon_source")
        wordLexiconTarget, wordEmbeddingTarget = Embedding.fromWord2Vec(args.word_embedding_target, "UUUNKKK",
                                                                        "word_lexicon_target")

        if args.word_embedding_intermediary:
            wordLexiconIntermediary, wordEmbeddingIntermediary = Embedding.fromWord2Vec(
                args.word_embedding_intermediary, "UUUNKKK",
                "word_lexicon_intermediary")
        else:
    ...
    if not normalizeMethod:
        ...
        wordEmbeddingSource.minMaxNormalization()
        wordEmbeddingTarget.minMaxNormalization()
        wordEmbeddingIntermediary.minMaxNormalization()
    ...
    if isSentenceModel:
        ...
        wordWindowSource = T.lmatrix("word_window_source")
        wordWindowIntermediary = T.lmatrix("word_window_intermediary")
        wordWindowTarget = T.lmatrix("word_window_target")
        inputModel = [wordWindowSource, wordWindowIntermediary, wordWindowTarget]

        wordEmbeddingLayerSource = EmbeddingLayer(wordWindowSource, wordEmbeddingSource.getEmbeddingMatrix(),
                                                  trainable=args.train_word_embedding,
                                                  name="word_embedding_layer_source")
        flattenSource = FlattenLayer(wordEmbeddingLayerSource)

        wordEmbeddingLayerIntermediary = EmbeddingLayer(wordWindowIntermediary,
                                                        wordEmbeddingIntermediary.getEmbeddingMatrix(),
                                                        trainable=args.train_word_embedding,
                                                        name="word_embedding_layer_intermediary")
        flattenIntermediary = FlattenLayer(wordEmbeddingLayerIntermediary)

        wordEmbeddingLayerTarget = EmbeddingLayer(wordWindowTarget, wordEmbeddingTarget.getEmbeddingMatrix(),
                                                  trainable=args.train_word_embedding,
                                                  name="word_embedding_layer_target")
    ...
    inputGenerators = [
        WordWindowGenerator(wordWindowSize, wordLexiconSource, wordFilters, startSymbol, endSymbol),
        WordWindowGenerator(wordWindowSize, wordLexiconIntermediary, wordFilters, startSymbol, endSymbol),
        WordWindowGenerator(wordWindowSize, wordLexiconTarget, wordFilters, startSymbol, endSymbol),
    ]
    ...
    log.info("Source: Size of  word dictionary and word embedding size: %d and %d" % (
        wordEmbeddingSource.getNumberOfVectors(), wordEmbeddingSource.getEmbeddingSize()))
    log.info("Intermediary: Size of  word dictionary and word embedding size: %d and %d" % (
        wordEmbeddingIntermediary.getNumberOfVectors(), wordEmbeddingIntermediary.getEmbeddingSize()))
    log.info("Target: Size of  word dictionary and word embedding size: %d and %d" % (
        wordEmbeddingTarget.getNumberOfVectors(), wordEmbeddingTarget.getEmbeddingSize()))
    if withCharWNN:
