...
class SentenceEmbedding(Layer):
    ...
    def getAllWordCharWindowIndexesBySentence(self, inputData):
        ...
        for idxSentence in xrange(len(inputData)):            
            for idxExample in xrange(len(inputData[idxSentence])):
                # Indexes of wordWindow within the window of the current word (idxExample).
                wordWindow = self.getWindowIndexes(idxExample, inputData[idxSentence])
