...
PARAMETERS = {
    ...
    "save_lexicon": {"desc": "save trained word lexicon to File Path"}
}
...
def main():
    ...
    def pour(miniNet, input):
        inGen = WordWindowGenerator(wordWindowSize, wordLexicon, filters, startSymbol, endSymbol)
        return miniNet(inGen(input))

    miniNet = theano.function(inputs=[inWords], outputs=[hiddenAct.getOutput()])

    hiddenAfterPour = pour(miniNet, "rt ##REF## : para a região das moreninhas , vamos recapear algumas ruas que estão em péssimo estado e colocar asfalto de qualidade . ##HASHTAG##")
    log.info(str(hiddenAfterPour))
