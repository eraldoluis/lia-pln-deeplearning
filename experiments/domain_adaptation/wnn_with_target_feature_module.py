...
WNN_PARAMETERS = {
    ...
    "target_module": {
        "required": True,
        "desc": "The file path which has the weights and parameters of the model"
    },

    "target_filters": {"required": True,
                       "desc": "a list which contains the filters. Each filter is describe by your module name + . + class name"},
...
def mainWnn(args):
    ...
    # Lendo Filtros do target
    log.info("Lendo filtros Target")
    targetFilters = getFilters(args.target_filters, log)
    ...
    if isSentenceModel:
        ...
        # Target Module
        targetInput = T.lmatrix("target_input")
        inputModel.append(targetInput)

        targetEmbeddingLayer = EmbeddingLayer(targetInput, targetWordEmbedding.getEmbeddingMatrix(),
                                              name="target_embedding", trainable=False)
        targetFlatten = FlattenLayer(targetEmbeddingLayer)

        if targetHiddenParameters:
            targetLinear = LinearLayer(targetFlatten, targetWindowSize * targetWordEmbedding.getEmbeddingSize(),
                                       targetHiddenSize, W=targetHiddenParameters[0], b=targetHiddenParameters[1],
                                       name="target_linear", trainable=False)
            targetAct = ActivationLayer(targetLinear, tanh)

            sizeLastLayerTarget = targetHiddenSize
            lastLayerTarget = targetAct
        else:
    ...
    # Load the model
    if args.load_model:
        alreadyLoaded = set([wordEmbeddingLayer, targetEmbeddingLayer, ])
    ...
    inputGenerators.append(WordWindowGenerator(targetWindowSize, targetLexicon, targetFilters, targetStartSymbol, None))
    outputGenerator = LabelGenerator(labelLexicon)
