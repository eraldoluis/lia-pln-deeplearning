...
class WindowModelBasic:
    ...
    def __init__(self, lexicon, wordVectors, windowSize, hiddenSize, _lr,
                 ...
                 structGrad=True, adaGrad=False, loadHiddenWeights=None):
        # Logging object.
        self.__log = logging.getLogger(__name__)
        ...
        self.__loadHiddenWeights = loadHiddenWeights
    ...
    def initWithBasicLayers(self, withoutHiddenLayer):
        ...
        self.concatenateEmbeddings = T.concatenate([embeddingLayer.getOutput() for embeddingLayer in self.embeddings],
                                                   axis=1)
        ...
        if not withoutHiddenLayer:
            ...
            if self.charModel == None:
                ...
                if self.__loadHiddenWeights:
                    f = h5py.File(self.__loadHiddenWeights, mode='r')
                    W = theano.shared(value=np.asarray(f['dense_1']['dense_1_W'], dtype=theano.config.floatX),
                                      name='W_hiddenLayer', borrow=True)
                    b = theano.shared(value=np.asarray(f['dense_1']['dense_1_b'], dtype=theano.config.floatX),
                                      name='b_hiddenLayer', borrow=True)
    ...
    def train(self, inputData, correctData, indexesOfRawWord):
        ...
        log.info(
            "Training with %d examples using %d mini-batches (batchSize=%d)..." % (numExs, numBatches, self.batchSize))
