...
WNN_PARAMETERS = {
    ...
    # Dataset
    "train_source": {"desc": "Training File Path"},
    "train_target": {"desc": "Training File Path"},
...
class SupervisedDAIterator(object):
    def __init__(self, sourceBatchIterator, targetBatchIterator, _lambda):
        self.__sourceBatchIterator = sourceBatchIterator
        self.__targetBatchIterator = targetBatchIterator

        sourceSize = sourceBatchIterator.size()
        targetSize = targetBatchIterator.size()

        # source_prob = |S| / (|S| + |T| * lambda * |S|/|T| ) = |S|/ |S| + lambda * |S|,
        #   where |S| is the size of the dataset of the source domain,
        #   |T| is the size of the dataset of the target domain,
        #   lambda is constant equal or greater than 0
        self.__sourceProbability = float(sourceSize) / (sourceSize + _lambda * sourceSize)

        # Maximun number of examples to be read per epoch
        self.__maxNmExPerEpoch = sourceSize + targetSize

        self.__numberExRead = 0
    ...
    def next(self):
        ...
        if self.__numberExRead == self.__maxNmExPerEpoch:
            self.__numberExRead = 0
            raise StopIteration()

        isToReadSourceEx = np.random.uniform() < self.__sourceProbability

        if isToReadSourceEx:
            i = np.random.randint(0, self.__sourceBatchIterator.size())
            example = self.__sourceBatchIterator.get(i)
        else:
            i = np.random.randint(0, self.__targetBatchIterator.size())
            example = self.__targetBatchIterator.get(i)

        self.__numberExRead += 1
...
def mainWnn(args):
    ...
    if args.train_source:
        ...
        sourceTrainDatasetReader = TokenLabelReader(args.train_source, args.token_label_separator)
        sourceBatchIterator = SyncBatchIterator(sourceTrainDatasetReader, inputGenerators, [outputGenerator], batchSize,
                                        shuffle=shuffle)

        targetTrainDatasetReader = TokenLabelReader(args.train_target, args.token_label_separator)
        targetBatchIterator = SyncBatchIterator(targetTrainDatasetReader, inputGenerators, [outputGenerator], batchSize,
                                                shuffle=shuffle)

        supervisedDAIterator = SupervisedDAIterator(sourceBatchIterator,targetBatchIterator,args.lambda_da)
    ...
    if supervisedDAIterator:
        ...
        log.info("Training")
        wnnModel.train(supervisedDAIterator, numEpochs, devReader, callbacks=callback)
